...
const RowContainer = ({flag, data, scrollValue}) => {
  ...
  useEffect(() => {
    rowContainer.current.srollLeft += scrollValue;
  },[scrollValue])
    ...
    className={`w-full flex items-center gap-3 my-12 scroll-smooth 
      ${flag ? 'overflow-x-scroll' : 'overflow-x-hidden flex-wrap justify-center'}`}>
       {data && data.length > 0 ? (data.map((item) => (
            <div key={item?.id} 
              className='w-300 h-[225px] min-w-[300px] md:w-340 md:min-w-[340px] my-12 rounded-lg bg-cardOverlay p-2 
            backdrop-blur-lg hover:drop-shadow-lg flex flex-col items-center justify-between'>
              ...
              <div className='w-full flex flex-col items-end justify-end'></div>
